...
//------------Retrieving all Items---------//
  getAllItems() {
    return this.http.get("http://localhost:8028/emart/item/all");
  }
...
//-------------Retrieving particular Item from Database--------//
  getItem(itemId) {
    return this.http.get("http://localhost:8028/emart/item/" + itemId);
  }
...
//----------Storing BillDetails------//
  addBill(todayDate: Date, total: number) {
    ...
    this.cartItems = [];
    return this.http.post("http://localhost:8028/emart/bitem", bill);
  }

//----------Retrieving CurrentBuyer Data-------//
  getBuyer() {
    return this.http.get("http://localhost:8028/emart/buyer/" + this.currentBuyer.id);
  }
...
//----------Buyer validation----------//
  validateBuyer(user: string, password: string) {
    ...
    return this.http.get("http://localhost:8028/emart/validate", { headers });
  }
